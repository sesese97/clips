...
export async function createProject(file?: File, youtubeUrl?: string): Promise<Project> {
  ...
  return json(await fetch(`${API}/api/projects`, { method: 'POST', body: fd }));
}

export async function getProject(id: string): Promise<Project> {
  return json(await fetch(`${API}/api/projects/${id}`));
}

export async function startTranscription(id: string) {
  return json(await fetch(`${API}/api/projects/${id}/transcribe`, { method: 'POST' }));
}

export async function searchTranscript(id: string, mode: 'keyword' | 'theme', query: string): Promise<SearchResult[]> {
  const data = await json<{results: SearchResult[]}>(await fetch(`${API}/api/projects/${id}/search`, {
...
export async function addMedia(id: string, file?: File, url?: string) {
  ...
  return json(await fetch(`${API}/api/projects/${id}/media`, {method:'POST', body:fd}));
}
...
export async function renderClip(id: string, body: {
  ...
  return json<{id:string;file:string}>(await fetch(`${API}/api/projects/${id}/render`, {
