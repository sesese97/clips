...
export default function App(){
  ...
  function setRange(a:number,b:number){
    ...
    const safeB=Math.max(safeA,Math.min(b,duration||b));
            ...
            {mode==='time'?
              <div>
                <div className="time-grid">
                  ...
                  <label>Fin<input type="text" inputMode="decimal" value={endText} placeholder="4:51" onChange={e=>editTime('end',e.target.value)} onBlur={()=>setEndText(sec(end))} onKeyDown={e=>{if(e.key==='Enter')setEndText(sec(end))}}/></label>
                  <div className={clipLen<=0||clipLen>30?'duration bad':'duration'}>{clipLen.toFixed(1)} s / 30 s</div>
                </div>
                <p style={{fontSize:12,opacity:.65,marginTop:8}}>Puedes escribir 4:21, 12:05.5 o 1:02:10. El clip final debe durar máximo 30 s.</p>
              </div>
              :<div>
                ...
                <div className="results">{results.map((r,i)=><button key={i} className="result" onClick={()=>setRange(r.start,Math.min(r.end,r.start+30))}><b>{sec(r.start)} – {sec(r.end)}</b><span>{r.text.slice(0,220)}</span><em>{Math.round(r.score*100)}%</em></button>)}</div>
              </div>}
